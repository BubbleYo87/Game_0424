...
public class Enemy_BoomB : MonoBehaviour
{
    ...
    [Header("UI 淡出設定")]
    [Tooltip("Boom UI 淡出時間 (秒)")]
    public float uiFadeOutTime = 0.5f;    // 新增：淡出所需時間
    ...
    // Patrol 相關
    private Coroutine patrolRoutine;         // 用來記錄正在跑的 Patrol Coroutine
    // ----- 新增私有欄位 -----
    ...
    void Awake()
    {
        ...
        explosionDamage = boomCollider.GetComponent<ExplosionDamage>();
    ...
    void Update()
    {
        ...
        if (childAnimator != null)
        {
            ...
            {
                // 如果 HandleChasing() 返回 true，表示已經開始追擊
                childAnimator.SetBool("BoomB_Chasing", true);
            }
            else
            {
                childAnimator.SetBool("BoomB_Chasing", false);
            }
            // 每幀同步速度參數
        ...
        // 狀態改變時：如果剛進 Idle 就啟動 Patrol；如果剛離開 Idle 就停止 Patrol
        if (currentState != lastState)
        {
            // 從其它狀態回到 Idle
            if (currentState == State.Idle && patrolRoutine == null)
            {
                patrolRoutine = StartCoroutine(PatrolRoutine());
            }
            // Idle → 其它狀態
            else if (lastState == State.Idle && patrolRoutine != null)
            {
...
/*     public void TakeDamage(int dmg = 1)
    {
        ...
        if (currentHP <= 0)
        {
            Destroy(gameObject, 5f);        // 血量歸零後 3 秒後銷毀物件
        }
    }
    ...
    private void SyncSpeedWithAnimator()
    {
        ...
        // 第三個參數是 dampTime (秒)，第四個參數是 deltaTime
        // 0.2f 秒的阻尼，表示大概 0.2 秒 63% 追到目標，0.6 秒追到 95%
        childAnimator.SetFloat("BoomB_Speed", currentMoveSpeed, 0.2f, Time.deltaTime);
    ...
    public void Boom()
    {
        ...
        LastDamageValue =  Round(Mathf.Lerp(1f, 100f, normalized), 1);
        Debug.Log($"[BoomUI] value = {LastDamageValue}");

        // 2) 先把傷害值傳給 ExplosionDamage（關鍵修正）
        if (explosionDamage != null)
        explosionDamage.SetDamage(LastDamageValue);
    ...
    private IEnumerator HandleBoomUIFeedback()
    {
        ...
        // 5. 倒數結束後，開始淡出
        float fadeTimer = 0f;
        float startAlpha = boomUIImage.color.a;  // 目前的透明度
        while (fadeTimer < uiFadeOutTime)
        {
            fadeTimer += Time.unscaledDeltaTime;
            // 線性插值從 startAlpha → 0
            float newAlpha = Mathf.Lerp(startAlpha, 0f, fadeTimer / uiFadeOutTime);
            c = boomUIImage.color;
            c.a = newAlpha;
            boomUIImage.color = c;
            yield return null;
    ...
    /// <summary>
    /// 掃視：轉向一個隨機的 yaw 角度 (±maxIdleYawAngle)，花 idleRotateDuration 秒完成
    /// </summary>
    private IEnumerator Scan()
    {
        float startY = transform.eulerAngles.y;
        float targetY = startY + Random.Range(-maxIdleYawAngle, maxIdleYawAngle);
        ...
        while (t < idleRotateDuration)
        {
            transform.rotation = Quaternion.Slerp(startRot, targetRot, t / idleRotateDuration);
            t += Time.deltaTime;
            yield return null;
        }
    ...
    /// <summary>
    /// 遊蕩：往「看著的方向」前進一段距離 (3~7f)，到達後結束
    /// </summary>
    private IEnumerator Wander()
    {
        // 如果不在 NavMesh 上，就直接結束此 Coroutine
        if (!agent.isOnNavMesh)
        {
            // Debug.LogWarning("[Enemy_BoomB] Wander 中發現 agent 不在 NavMesh，已取消 Wandering");
            yield break;
        }
        ...
        NavMeshHit navHit;
        Vector3 targetPos = NavMesh.SamplePosition(rawTarget, out navHit, 1f, agent.areaMask)
                            ? navHit.position
                            : rawTarget;
        ...
        float timeout = 5f;
        // 這裡就不會再拋「GetRemainingDistance」的錯
        while (!agent.pathPending && agent.remainingDistance > 0.1f && timeout > 0f)
        {
    ...
    /// <summary>
    /// Patrol 主流程：Idle 時不斷「掃視→(80%)遊蕩→等待」循環
    /// </summary>
    private IEnumerator PatrolRoutine()
    {
        ...
        // 一旦 currentState != Idle，就自動結束 Patrol
        patrolRoutine = null;
    }
    // 四捨五入
